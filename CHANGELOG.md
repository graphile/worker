...
>
> Worker Pro is a proprietary plugin which, among other things, helps to
> alleviate this issue. It tracks the running workers and when a new breaking
> migration is required it has all old workers exit cleanly (once they've
> finished what they're working on) and no further old workers will be able to
> start up. Once all old workers have exited, the migration can go ahead as it
> normally would. This avoids the need to "scale to zero" as workers will
> communicate with each other to make sure the system as a whole remains
> consistent.
>
> Read more:
