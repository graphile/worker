...
`,
  ...
  await withPgClient(async (pgClient) => {
    ...
    expect(migrationRows.length).toBeGreaterThanOrEqual(14);
...
test("aborts if database is more up to date than current worker", async () => {
  ...
  await withPgClient(async (pgClient) => {
    ...
    await expect(
      ...
      `"Database is using Graphile Worker schema revision 999999 which includes breaking migration 999999, but the currently running worker only supports up to revision 14. It would be unsafe to continue; please ensure all versions of Graphile Worker are compatible."`,
    );
  });
});
