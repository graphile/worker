...
export const initWorker = async (
  ...
  assert(
    typeof poolOrConfig !== "undefined",
    "The first argument to graphile-worker was `undefined`... did you mean to set pool options?"
  );
  ...
  assert(
    typeof jobs === "number" && jobs >= 1,
    "jobs option should be a valid number greater or equal to 1"
  );
  assert(
    typeof pollInterval === "number" && pollInterval >= 0,
    "pollInterval option should be a valid number greater than 0"
  );

  assert(
    (taskDirectory && !taskList) || (!taskDirectory && taskList),
...
export class Worker {
  ...
  constructor({
    ...
    (this.pgPool = pgPool), (this.taskList = taskList);
